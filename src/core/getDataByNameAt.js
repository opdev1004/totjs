...
export async function getDataByNameAt(filename, name, position = 0, encoding = 'utf8', streamCount = DefaultStreamCount)
{
    try
    {
        if (!name)
        ...
        const result = await processGetDataByNameAt(filename, name, position, encoding, streamCount);
        return result;
    } catch (error)
    {
...
async function processGetDataByNameAt(filename, name, position, encoding, streamCount)
{
